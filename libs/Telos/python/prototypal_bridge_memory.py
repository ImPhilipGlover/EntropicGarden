...
# Locate the telos package directory
_telos_dir = Path(__file__).parent.parent / "telos"
_bridge_module_path = _telos_dir / "_telos_bridge.cpython-311-x86_64-linux-gnu.so"

# Load the CFFI bridge module directly
try:
    spec = importlib.util.spec_from_file_location("_telos_bridge", _bridge_module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load CFFI bridge from {_bridge_module_path}")
    
    _telos_bridge = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(_telos_bridge)
    ffi = _telos_bridge.ffi
    lib = _telos_bridge.lib
except Exception as memory_error:
    print(f"CRITICAL ERROR: Real CFFI bridge import failed - cannot proceed without functional C ABI", file=sys.stderr)
    print(f"Bridge module path: {_bridge_module_path}", file=sys.stderr)
    print(f"Import error: {memory_error}", file=sys.stderr)
    raise RuntimeError("TELOS requires functional C ABI - cannot proceed without real implementation") from memory_error
