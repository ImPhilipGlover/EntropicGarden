"""
Transactional Outbox Handlers for TELOS Workers

This module contains handlers for transactional outbox operations,
...
def handle_transactional_outbox(worker_instance, request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == 'run_scenario':
        return _transactional_outbox_run_scenario(worker_instance, config)

    if action in ('dlq_snapshot', 'dlq_inspect'):
        return _transactional_outbox_dlq_snapshot(worker_instance, config)

    if action == 'purge_processed':
        return _transactional_outbox_purge_processed(worker_instance, config)

    if action == 'enqueue_matrix':
        return _transactional_outbox_enqueue_matrix(worker_instance, config)
...
def _transactional_outbox_run_scenario(worker_instance, config: Dict[str, Any]) -> Dict[str, Any]:
...
def _transactional_outbox_dlq_snapshot(worker_instance, config: Dict[str, Any]) -> Dict[str, Any]:
...
def _transactional_outbox_purge_processed(worker_instance, config: Dict[str, Any]) -> Dict[str, Any]:
...
def _transactional_outbox_enqueue_matrix(worker_instance, config: Dict[str, Any]) -> Dict[str, Any]:
