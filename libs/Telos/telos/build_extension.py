...
try:
    import uvm_object
    create_uvm_object = uvm_object.create_uvm_object
except ImportError:
    print(f"ERROR: Could not import uvm_object from {python_dir}", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    declarations = """
        ...
        BridgeConfig* bridge_create_config(int max_workers, const char* log_level, const char* log_file, size_t shared_memory_size, const char* worker_path);
        ...
        // Simplified shared memory functions (Io-compatible)
        ...
        BridgeResult bridge_submit_json_task_simple(const char* request_json, char* response_buffer, size_t buffer_size);
