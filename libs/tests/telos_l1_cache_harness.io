...
TelosL1CacheHarness := Object clone do(
    ...
    run := method(
        ...
        // Only stop Telos if it was successfully started
        if(Telos Bridge status at("initialized"),
            Telos stop
        )
        Telos Bridge clearError
...
if(result type == "Exception",
    writeln("TelosL1CacheHarness failure: " .. result message)
    result pass
)

writeln("TelosL1CacheHarness success")
