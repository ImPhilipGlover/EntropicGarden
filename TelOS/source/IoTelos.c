...
// Append a morph directly to the C world's submorph list so C draw/loop can render it
IoObject *IoTelos_addMorphToWorld(IoTelos *self, IoObject *locals, IoMessage *m)
{
    if (!globalWorld || !globalWorld->world) {
        printf("Telos: No world exists - call createWorld first\n");
        return self;
    }

    IoObject *childMorph = IoMessage_locals_valueArgAt_(m, locals, 0);
    if (!childMorph) {
        printf("Telos: Invalid child morph for addMorphToWorld\n");
        return self;
    }
    ...
    if (!submorphs) {
        ...
        globalWorld->world->submorphs = submorphs;
    }
    IoList_rawAppend_(submorphs, childMorph);
    printf("Telos: Morph appended to C world (visible to draw loop)\n");
    return self;
}
