...
class GlobalState:
    ...
    def __init__(self, log_handle, signals_instance):
        self.log_handle = log_handle
        self.signals = signals_instance # Injected signals instance
...
def load_file_content(filepath, is_critical=True, default_content=""):
    ...
    try:
        ...
        if is_critical and global_state:
            # Emit critical error via global_state.signals
...
def alfred_assess_stagnation_and_chaos(session_messages):
    ...
    if len(global_state.conceptual_velocity_history) >= Config.STAGNATION_THRESHOLD:
        ...
        if recent_average_variance < 0.3:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED: {msg}")
        else:
...
def alfred_log_google_query(response_content):
    ...
    if matches:
        ...
        try:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Failed to log Google query: {e}")
    return []
...
def alfred_extract_and_log_proposals(session_messages):
    ...
    if unique_proposals:
        try:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Failed to log proposals: {e}")
    else:
    ...
    try:
        ...
        if proposal:
            try:
                ...
                global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Failed to save knowledge chunk: {e}")
    except Exception as e:
        global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Error proposing knowledge chunk: {e}")
...
def perform_stylistic_audit(response_content):
    ...
    global_state.signals.alfred_signal.emit(f"ALFRED: {audit_log_content}")
...
def alfred_suggest_heartbeat_adjustment(current_heartbeat, last_duration, cycle_num):
    ...
    if cycle_num > 1 and last_duration > 0:
        ...
        if message:
            global_state.signals.alfred_signal.emit(f"ALFRED: {message}")
...
def alfred_check_and_inject_user_feedback():
    ...
    if feedback_content:
        try:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Error clearing user feedback file: {e}. Manual clear recommended.")
...
def alfred_analyze_and_store_sentiment():
    ...
    if global_state.current_sentiment != dominant_sentiment:
        ...
        global_state.signals.alfred_signal.emit(f"ALFRED: Sentiment analysis complete. Prevailing sentiment detected: '{dominant_sentiment}'.")
        global_state.signals.sentiment_changed_signal.emit(dominant_sentiment)
    else:
        global_state.signals.alfred_signal.emit(f"ALFRED: Sentiment remains: '{dominant_sentiment}'. No change.")
...
def alfred_find_and_set_next_theme(current_theme):
    ...
    try:
        ...
        global_state.signals.alfred_signal.emit(f"ALFRED: Thematic resonance analysis complete. Next theme set to: '{next_theme}'.")
        global_state.signals.theme_changed_signal.emit(next_theme)
    except Exception as e:
        global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Failed to set next theme: {e}")
    return next_theme
...
def generate_end_of_session_report(persona_codex, full_dialogue):
    ...
    if not global_state: return
    global_state.signals.alfred_signal.emit("\n" + "="*20 + " END OF SESSION REPORT " + "="*20)
    ...
    try:
        ...
        try:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED: Next session directive saved: '{directive}'")
        except Exception as e:
            global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Failed to save directive: {e}")

    except Exception as e:
        global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Report generation failed: {e}")
    ...
    try:
        ...
        try:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Failed to save loom metaphor: {e}")
        ...
        global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): The Loom process failed: {e}")

def alfred_summarize_forge_item(persona_codex):
    """
    At the end of a 7-cycle, BRICK summarizes one of his proposed Forge items.
    Includes robust error handling for potentially malformed Forge protocols.
    """
    if not global_state: return
    forge_protocols = load_json_file(Config.THE_FORGE_FILE)

    if not forge_protocols:
        global_state.signals.alfred_signal.emit("ALFRED: No Forge protocols found to summarize or file is empty/corrupted.")
        return
    ...
    if not valid_protocols:
        global_state.signals.alfred_signal.emit("ALFRED: No valid Forge protocols found (missing 'name' or 'description') to summarize after filtering.")
        return
    ...
    try:
        ...
        # Emit the summary as an assistant dialogue for the main pane
        global_state.signals.dialogue_signal.emit("BRICK_FORGE", summary_content)
        
        append_to_log(global_state.log_handle, {"timestamp": str(datetime.datetime.now()), "role": "BRICK_FORGE_SUMMARY", "content": summary_content})
    except Exception as e:
        global_state.signals.alfred_signal.emit(f"ALFRED (ERROR): Error generating Forge summary for '{protocol_name}': {e}.")
...
def _build_chaos_injection(cycle_num, current_concept, current_context, concepts_lines, contexts_lines):
    ...
    if cycle_num > 1 and random.random() < global_state.chaos_probability:
        ...
        if choice_type == 'concept_context' and concepts_lines and contexts_lines:
            ...
            global_state.signals.alfred_signal.emit(f"ALFRED: {msg.strip()}")
            return f"\n{msg}"
...
class AlchemyEngine(QObject):
    ...
    def __init__(self, signals_instance):
        ...
        if len(self.session_messages_full_history) > 1:
            last_message = self.session_messages_full_history[-1]['content']
            self.signals.alfred_signal.emit(f"ALFRED: Resuming from prior state. Last thought: '{last_message[:80]}...'")
    ...
    def update_chaos(self, value):
        if global_state:
            ...
            self.signals.alfred_signal.emit(f"ALFRED: Chaos Probability set to {global_state.chaos_probability:.2f}.")
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            if self.user_input_queue:
                ...
                self.session_messages_full_history.append({'role': 'user', 'content': user_text})
                append_to_log(global_state.log_handle, {'timestamp': str(datetime.datetime.now()), 'role': 'user', 'content': user_text})
                ...
                try:
                    ...
                    # Use regex to split the text by speaker, keeping the delimiters.
                    # This pattern needs to match how the personas are instructed to prefix their speech.
                    # The regex needs to be non-greedy and handle potential trailing newlines.
                    
                    # Updated regex to correctly capture persona prefixes
                    # It looks for 'ROBIN:', 'BRICK:', or 'BRICK (Forge Summary):'
                    # and captures the content following it.
                    
                    segments = re.split(r'(ROBIN:|BRICK:|BRICK \(Forge Summary\):)', full_assistant_content, flags=re.IGNORECASE)
                    
                    # The first segment is usually empty or general dialogue before a specific persona starts.
                    # We'll treat any leading text before a recognized persona as general assistant output.
                    if segments[0].strip():
                        self.signals.dialogue_signal.emit("assistant", segments[0].strip())
                        
                    # Iterate through the split segments, which come in pairs: (delimiter, content)
                    for i in range(1, len(segments), 2):
                        speaker_prefix = segments[i].strip()
                        speaker_content = segments[i+1].strip() if i+1 < len(segments) else ""
                        
                        role_to_emit = "assistant" # Default, or for "B&R"
                        
                        if speaker_prefix.lower() == "robin:":
                            role_to_emit = "ROBIN"
                        elif speaker_prefix.lower() == "brick:":
                            role_to_emit = "BRICK"
                        elif speaker_prefix.lower() == "brick (forge summary):": # Case-sensitive matching is safer here for the specific prefix
                            role_to_emit = "BRICK_FORGE"
                    ...
                    # --- Rest of existing code for post-response analysis and error handling ---
                except Exception as e:
            ...
            self.signals.alfred_signal.emit(f"ALFRED: New epoch. THEME: '{self.current_theme}'. MODE: '{session_operational_mode}'.")
            
            if global_state.session_counter % Config.SENTIMENT_ANALYSIS_FREQUENCY_SESSIONS == 0:
                alfred_analyze_and_store_sentiment()
            ...
            for i in range(1, Config.RECURSIVE_CYCLES + 1):
                ...
                else: # Default behavior for other modes
                ...
                # ***FIX IS HERE***: Correctly call the now-defined function with the right arguments
                ...
                try:
                    self.signals.engine_status_signal.emit("Contacting LLM...")
                    ...
                    for i_segment in range(1, len(segments), 2): # Changed loop variable to avoid conflict with outer 'i'
                        speaker_prefix = segments[i_segment].strip()
                        speaker_content = segments[i_segment+1].strip() if i_segment+1 < len(segments) else ""
                        
                        role_to_emit = "assistant" 
                        
                        if speaker_prefix.lower() == "robin:":
                            role_to_emit = "ROBIN"
                        elif speaker_prefix.lower() == "brick:":
                            role_to_emit = "BRICK"
                        elif speaker_prefix.lower() == "brick (forge summary):":
                            role_to_emit = "BRICK_FORGE"
                    ...
                    perform_stylistic_audit(new_thought_obj['content'])
                    alfred_log_google_query(new_thought_obj['content'])

                except Exception as e:
                    self.signals.alfred_signal.emit(f"ALFRED (ERROR): Ollama call failed in cycle {i}: {e}")
                    if len(current_session_messages_for_ollama) > 1: current_session_messages_for_ollama.pop()
                    break
            ...
            self.signals.alfred_signal.emit(f"----- ALFRED: 7-cycle session for (Concept: {current_concept}, Context: {current_context}) concluded. -----")
            ...
            alfred_summarize_forge_item(self.persona_codex)

            time.sleep(Config.HEARTBEAT_INTERVAL_SECONDS * 2)

            if time.time() >= epoch_end_time:
                self.signals.alfred_signal.emit(f"ALFRED: Thematic epoch concluded. Transitioning to next theme.")
                self.current_theme = alfred_find_and_set_next_theme(self.current_theme)

        self.signals.engine_status_signal.emit("Engine loop terminated.")